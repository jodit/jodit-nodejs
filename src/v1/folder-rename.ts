...
export async function folderRenameHandler(
  ...
  const queryValidation = FolderRenameQuerySchema.safeParse(req.query);
  ...
  logger.debug(
    `Renaming folder ${query.name} to ${query.newname} in ${sourceName}${query.path ?? '/'}`
  );

  // Construct paths
  const requestPath = query.path ?? '/';
  const parentDir = path.join(sourceConfig.root, requestPath);
