 ...
 */
function luminate(color: RGBColor, percent: number): RGBColor {
  const result: RGBColor = [0, 0, 0];

  for (let i = 0; i < color.length; i++) {
    const value = luminateValue(color[i] as number, percent);
 ...
 */
function luminateValue(value: number, percent: number): number {
...
export function generateIcon(
  ...
  const code = word.charCodeAt(0) % colors.length;
  const color = colors[code] as RGBColor;

  // Generate color variations (lines 109-118)
  const darkColor = luminate(color, 30);
  const shadowColor = luminate(color, 45);

  const main = fromRGB(color[0], color[1], color[2]);
  const dark = fromRGB(darkColor[0], darkColor[1], darkColor[2]);
  const shadow = fromRGB(shadowColor[0], shadowColor[1], shadowColor[2]);
